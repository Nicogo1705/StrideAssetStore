...
namespace StrideAssetStore.Core.Local.Registry;

/// <summary>
/// Opens registry pull requests through the GitHub CLI: fork → branch → write
/// <c>registry/&lt;id&gt;.json</c> → pull request, authenticated by the user's existing
/// <c>gh auth login</c> so no token is ever entered anywhere.
/// </summary>
/// <remarks>
/// Lives here rather than with the desktop app because the CLI submits the same pull requests —
/// `publish`, `certify`, `deprecate`, `unpublish` — and two implementations of a flow this
/// fiddly (renamed forks, stale forks, asynchronous fork creation) would diverge on the first bug
/// fixed in one of them.
/// </remarks>
/// <param name="owner">Registry repository owner.</param>
/// <param name="repo">Registry repository name.</param>
/// <param name="baseBranch">Branch the pull requests target.</param>
public sealed class RegistryPublisher(string owner, string repo, string baseBranch)
{
    private readonly string _owner = owner;
    private readonly string _repo = repo;
    private readonly string _base = baseBranch;
